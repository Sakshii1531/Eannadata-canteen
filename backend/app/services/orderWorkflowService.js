...
function deliveryBroadcastPayloadFromOrder(order, extra = {}) {
  ...
  return {
    ...
    preview: {
      ...
      paymentMode: order.paymentMode || order.payment?.method || "COD",
      codAmount: Math.max(0, (order.paymentBreakdown?.grandTotal ?? order.pricing?.total ?? 0) - (order.paymentBreakdown?.walletAmount ?? order.pricing?.walletAmount ?? 0)),
    },
